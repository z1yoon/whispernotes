...
const TranscriptPage = () => {
  ...
  const copySegmentText = async (text, segmentId) => {
    try {
  ...
  const getSpeakerInitials = (speakerName) => {
    if (speakerName && speakerName.includes('(')) {
      return speakerName.split('(')[0].trim().split(' ').map(n => n[0]).join('');
    }
    return speakerName?.split(' ').map(n => n[0]).join('') || 'S';
  };
