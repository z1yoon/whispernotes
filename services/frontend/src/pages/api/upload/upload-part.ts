...
export default async function uploadPart(req: NextApiRequest, res: NextApiResponse) {
  if (req.method !== 'POST') {
  ...
  try {
    // Get the session ID and part number from the request
    ...
    if (!partNumber || typeof partNumber !== 'string') {
      return res.status(400).json({ message: 'Missing partNumber parameter' });
    }
    ...
    console.log(`API: Upload part ${partNumber} for session ${sessionId}, file: ${file.originalFilename}, size: ${file.size} bytes`);
    
    // Create a form data object to send to the file-uploader service
    const formData = new FormData();
    formData.append('file', fs.createReadStream(file.filepath), {
      filename: file.originalFilename || `part-${partNumber}`,
    ...
    console.log(`API: Part ${partNumber} upload response:`, uploaderResponse.data);
    ...
    console.error(`API: Error in part ${req.query.partNumber} upload:`, uploadError.message);
